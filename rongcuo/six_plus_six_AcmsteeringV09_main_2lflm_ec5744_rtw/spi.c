...
void ec_spi1_init(void)
{
  ...
  DSPI(1).MODE.CTAR[0].B.CPOL = 0;     /* Clock polarity, 0 means the inactive state of SCK is low, 1 means high */
  DSPI(1).MODE.CTAR[0].B.CPHA = 1;     /* Clock phase, 0 means data is captured on the leading edge of SCK */
  DSPI(1).MODE.CTAR[0].B.LSBFE = 0;    /* LSB first enable, 0: MSB first; 1: LSB first */
